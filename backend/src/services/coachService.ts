...
export const getCoaches = async (tenantId: string) =>
  withTenantTransaction(tenantId, async (client) => {
    const result = await client.query('SELECT * FROM coaches WHERE tenant_id = $1', [tenantId]);
    return result.rows;
  });
...
export const deleteCoach = async (tenantId: string, id: string) =>
  withTenantTransaction(tenantId, async (client) => {
  const result = await client.query('DELETE FROM coaches WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
  if (result.rowCount === 0) throw new Error('Entrenador no encontrado');
  });